...
/**
 * VehicleMasterController implements the CRUD actions for VehicleMaster model.
 */
class VehicleMasterController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {  // echo "<pre>"; print_r($_POST); die;
           if(Yii::$app->request->isAjax){   
            ...
            $model->vehicle_name = strtoupper($_POST['VehicleMaster']['vehicle_name']);
            $model->reg_no = str_replace(' ','', strtoupper(trim($_POST['VehicleMaster']['reg_no'])));
            $model->driver_id = $_POST['VehicleMaster']['driver_id'];
            //  $cl = implode(',', $_POST['VehicleMaster']['client_id']);
            $cl='';
             if($_POST['VehicleMaster']['client_id']!=""){
             $cl = implode(',', $_POST['VehicleMaster']['client_id']);
             }
            $model->client_id = $cl;
            //$model->client_id = $_POST['VehicleMaster']['client_id'];
            // $model->user_id = $session['user_id'];
            // if($_POST['VehicleMaster']['status']==0){
            //     $model->status = "Inactive";
            // }else{
            //     $model->status = "Active";
            // } 

            if(array_key_exists('VehicleMaster', $_POST)){
              if(array_key_exists('status', $_POST['VehicleMaster'])){
                $model->status = "Active";
              }else{
                $model->status = "Inactive";
              }

              if(array_key_exists('is_trip_asign', $_POST['VehicleMaster'])){
                $model->is_trip_asign = "yes";
              }else{
                $model->is_trip_asign = "no";
              }
             }

            $model->updated_ipaddress =$_SERVER['REMOTE_ADDR'];
            $model->fc_expire_date = date('Y-m-d',strtotime($_POST['VehicleMaster']['fc_expire_date']));
            $model->ins_expired_date = date('Y-m-d',strtotime($_POST['VehicleMaster']['ins_expired_date']));
             if($_FILES['VehicleMaster']['error']['insurance_copy']==0)
            {
            ...
            if($_FILES['VehicleMaster']['error']['rc_book']==0)
            {
                  $rand = rand(0, 999); // random number generation for unique image save
             ...
             if($_FILES['VehicleMaster']['error']['tax_copy']==0)
            {
                  $rand = rand(0, 999); // random number generation for unique image save
            ...
            if($model->save()){
              if(!empty($_POST['VehicleMaster']['client_id'])){  //echo "<pre>"; print_r($_POST); die;
        $mapcl=ClientVehicleMap::find()->where(['vehicle_id'=>$model->id])->asArray()->all();
            $mapclAr=ArrayHelper::map($mapcl,'client_id','client_id');

            $postAr= $_POST['VehicleMaster']['client_id'];     
               
        foreach ($_POST['VehicleMaster']['client_id'] as $key => $value) {
        ...
        foreach ($mapclAr as $key => $value) {
         $ty=array_search($value, $cl);
          ...
          if(!array_search($value, $_POST['VehicleMaster']['client_id'])){ //echo "<pre>"; print_r($ty); 
            ...
            $ClientVehicle_1->updated_ipaddress = $_SERVER['REMOTE_ADDR'];
              ...
              $models= VehicleMaster::find()->where(['id'=>$autoid])->asArray()->one();
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
           if(Yii::$app->request->isAjax){ 
            ...
            // }
            // $model->is_trip_asign = 'yes';
            // if($_POST['VehicleMaster']['is_trip_asign']==0){
            //   $model->is_trip_asign = 'no';
            // }
    ...
    {
        if (($model = VehicleMaster::findOne($id)) !== null) {
            return $model;
        } else {
        ...
        public function actionAjaxfetch()
    {
        ...
        {
            $VehicleMaster=VehicleMaster::find()->select(['id'=>'id','vehicle_name'=>'vehicle_name'])->where(['status'=>'Active'])->andWhere(['LIKE','vehicle_name',$_POST['query']])->asArray()->all();
    ...
    public function actionVehicleMapping()
    {
        ...
        if ((Yii::$app->request->post())) { // echo "<pre>"; print_r($_POST); die;
            $driver_id = $_POST['VehicleMaster']['driver_id'];
            $vehi_id = $_POST['getid'];
            $dri_id='';
        
          //  $driverprofile = DriverProfile::findOne($driver_id);
            $vehiclemaster = VehicleMaster::findOne($vehi_id);
            if(!empty($vehiclemaster)){
    ...
    public function actionRemove($id)
    {
      ...
      // echo $id; die;
        $session = Yii::$app->session;
         $vehiclemaster = VehicleMaster::findOne($id);
            if(!empty($vehiclemaster)){ 
                $dri_id = $vehiclemaster->driver_id;  
                $driverprofile = DriverProfile::findOne($dri_id);  
    ...
    public function actionClientMapping()
    {
        ...
        if ((Yii::$app->request->post())) { //echo "<pre>"; print_r($_POST); die;
            $client_id = implode(',' ,$_POST['VehicleMaster']['client_id']);
            $vehi_id = $_POST['getid'];
            $dri_id=''; 
            $vehiclemaster = VehicleMaster::findOne($vehi_id);
            ...
            if(!empty($_POST['VehicleMaster']['client_id'])){  
  ...
  public function actionVehiDetails()
  {
    if($_POST){
      $vehinum = $_POST['regno'];
      $newregf = VehicleMaster::find()->where(['reg_no'=>$vehinum])->asArray()->one(); 
     //   echo "<pre>"; print_r($newregf); die;
      if(!empty($newregf)){
        $dericvprof = DriverProfile::find()->where(['id'=>$newregf['driver_id']])->asArray()->one();
        return json_encode($dericvprof);
      }
    }
  }
}
